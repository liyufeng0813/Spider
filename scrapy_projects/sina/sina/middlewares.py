...
class UASwitchDownloadMiddleware(object):
    def process_request(self, request, spider):
        ua_list = settings.USER_AGENT_LIST
        request.headers['User-Agent'] = random.choice(ua_list)


class IPSwitchDownloadMiddleware(object):
    def process_request(self, request, spider):
        r = requests.get('http://192.168.1.100:5001/get_proxy')
        ip = 'http://' + ':'.join(json.loads(r.text)[0])
